...
import { SvgArraw, SvgProfile, EditIcon } from './SvgComponents'

export const PrimaryButton = ({ lable, className, onClick }) => {
    return (
        <div className="button-borders">
            <button className={className || "primary-button"} onClick={onClick}> {lable}
            </button>
        </div>
    )
}
                ...
                <div className="submenu">
                    {
                        menuItems.map((item) => (
                            <div key={item} className="submenu-item">
                                <button id={item} onClick={() => onClick(item)} className="submenu-link">{item}</button>
                            </div>
                        ))
                    }
                </div>
